...
import type { TicketHelpdesk } from "../Models/HelpDesk";

export class TicketHelpdeskService {
  ...
  private toModel(item: any): TicketHelpdesk {
    const f = item?.fields ?? {};
    return {
  ...
  // ---------- CRUD ----------
  async create(record: Omit<TicketHelpdesk, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<TicketHelpdesk, 'ID'>>) {
        await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<PageResult<TicketHelpdesk>> {
    await this.ensureIds();
  ...
  async getByNextLink(nextLink: string): Promise<PageResult<TicketHelpdesk>> {
    return this.fetchPage(nextLink, /*isAbsolute*/ true);
  }

  private async fetchPage(url: string, isAbsolute = false): Promise<PageResult<TicketHelpdesk>> {
