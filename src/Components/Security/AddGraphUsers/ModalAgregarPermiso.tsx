...
import type { newAccess } from '../../../Models/SharepointGroups';


type Props = {
  isOpen: boolean;
  onClose: () => void;
  onSave?: (c: { name: string; mail: string }) => Promise<void> | void;
...
const ModalOtorgarPermiso: React.FC<Props> = ({isOpen, onClose, onSave, workers = [], workersLoading = false,}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      setLocalError(null);
      await onSave?.({  name: form.name, mail: form.mail });
      onClose();
    } catch (err: any) {
