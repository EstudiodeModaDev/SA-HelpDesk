...
export class LogHelpDeskService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): LogHelpdesk {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f.Title,
  ...
  // ---------- CRUD ----------
  async create(record: Omit<LogHelpdesk, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<LogHelpdesk, 'ID'>>) {
    await this.ensureIds();
